...
public class ChatRoomAsyncTask extends AsyncTask<String, Void, String> {
    //"http://192.168.1.67:8080/DemoServer/UrlController?action=" + "join";
    //"http://52.198.163.90:8080/DemoServer/UrlController?action=" + "join";

    public static String URL = "http://52.198.163.90:8080/DemoServer/FCMController?action=" + "wantJoin";
    ...
    public ChatRoomAsyncTask(int myAid, Activity context, String action){
        this.actId = myAid;
        mContext = context;
        this.action = action;

    }
    public ChatRoomAsyncTask(int myAid, Activity context, String action, String myJoinId){
        this.actId = myAid;
        mContext = context;
        this.action = action;
        this.joinId = myJoinId;
    }
    ...
    private InputStream getHttpConnection(String urlString)
        ...
        try {
            ...
            String params = "";
            if (action.equals("wantjoin")){
                params = jFactory.wantJoinStringfy(actId, CPublicParameters.user.getfUserName());
                params = String.format("json=%s", params);
            }else if(action.equals("agree")){
                params = jFactory.agreeRefuseStringfy(actId, joinId);
                params = String.format("json=%s", params);
